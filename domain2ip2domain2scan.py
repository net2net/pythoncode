...
def nmap_scan_result(path):
    dom = xml.dom.minidom.parse(path)
    root = dom.documentElement
    try:
        item = root.getElementsByTagName('address')
        ip=item[0].getAttribute('addr')
        itemlist = root.getElementsByTagName('port')
    except:
        nmapdict[scanip]='No port info.'
	return
    result=''
    for i in itemlist:
        b1=i.childNodes[0]
        b2=i.childNodes[1]
	result=result+i.getAttribute('portid')+ '    '+b1.getAttribute('state')+'    '+b2.getAttribute('name')+'\n'
    nmapdict[ip]=result
...
if __name__ == "__main__":   
    ...
    #iplist_sort=['127.0.0.1','192.168.40.200  192.168.40.222']
    if os.path.exists('hostinfo')!=True:
        os.mkdir('hostinfo') 
    for ip in iplist_sort:
        for j in ip.split(' '):
            print 'scaning ip:%s'%(j)
	    status,output=commands.getstatusoutput('nmap -Pn -n -sT --open %s -oX ./hostinfo/%s.xml'%(j,j))
    print "all ip scan over!"
    
    for ip in iplist_sort:
        for j in ip.split(' '):
	    nmap_scan_result('./hostinfo/%s.xml'%(j))

    for ip in iplist_sort:
        print '#########################################'
        #print 'ip:%s      \n%s\nnmap scan result:\n%s'%(ip,ipdict[ip],nmapdict[j])
	for j in ip.split(' '):
            print 'ip:%s      \n%s\nnmap scan result:\n%s'%(ip,ipdict[ip],nmapdict[j])
	    file('info_ipneighbor.txt','a+').write('\n##############################\nip:%s   \n%s\nnmap scan result:%s'%(ip,ipdict[ip],nmapdict[j]))
'''
iplist_sort=['127.0.0.1','192.168.40.200  192.168.40.222']
if os.path.exists('hostinfo')!=True:
    os.mkdir('hostinfo') 
